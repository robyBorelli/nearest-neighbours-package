...
#include "points.c"
float (*distanceMetric) (float*, float*, int) = &periodicDistance;
...
float* (*choseVantagePoint) (float**, int) = &choseRandomizedOptimalVantagePoint;
#include "vptree.c"
...
int main(int argc, char **argv){
    ...
    if(mode->setLowerBound == FALSE){
        ...
        for(int i = 0; i < d; i++){
            lowerBounds[i] = findBestProjectedPoint(records,n,d,i,strongMinComparison);
        }
    }else{
    ...
    if(mode->setUpperBound == FALSE){
        ...
        for(int i = 0; i < d; i++){
            upperBounds[i] = findBestProjectedPoint(records,n,d,i,strongMaxComparison);
        }
    }else{
    ...
    if(mode->entropyMode == TRUE){
        calcEntropy(records,res,lowerBounds,upperBounds,d,m,n);
    }
